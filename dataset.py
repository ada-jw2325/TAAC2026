...
class PCVRParquetDataset(IterableDataset):
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        # ---- Time features appended to user_dense (5 dims) ----
        self._num_time_feats = 5
        self.user_dense_schema.add(9901, 1)  # hour_sin
        self.user_dense_schema.add(9902, 1)  # hour_cos
        self.user_dense_schema.add(9903, 1)  # weekday_sin
        self.user_dense_schema.add(9904, 1)  # weekday_cos
        self.user_dense_schema.add(9905, 1)  # is_weekend
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- Time features from timestamp (vectorized) ----
        time_feat_offset = self.user_dense_schema.total_dim - self._num_time_feats
        valid_ts = timestamps > 0
        if valid_ts.any():
            seconds_of_day = timestamps[valid_ts] % 86400
            hours = seconds_of_day / 3600.0
            days_since_epoch = timestamps[valid_ts] // 86400
            weekdays = (days_since_epoch + 3) % 7  # 0=Mon, 6=Sun
            user_dense[valid_ts, time_feat_offset] = np.sin(2 * np.pi * hours / 24)
            user_dense[valid_ts, time_feat_offset + 1] = np.cos(2 * np.pi * hours / 24)
            user_dense[valid_ts, time_feat_offset + 2] = np.sin(2 * np.pi * weekdays / 7)
            user_dense[valid_ts, time_feat_offset + 3] = np.cos(2 * np.pi * weekdays / 7)
            user_dense[valid_ts, time_feat_offset + 4] = (weekdays >= 5).astype(np.float32)
