...
class TransformerEncoder(nn.Module):
    ...
    Supports optional semi-local causal mask (+1.5k eval AUC).
    """
    ...
    def forward(
        ...
        Returns:
            Tuple of (output tensor of shape (B, L, D), key_padding_mask).
        """
        # Self-Attention (Pre-LN) with RoPE + optional causal mask
        residual = x
        x = self.norm1(x)
        attn_mask = None
        if self.causal:
...
def create_sequence_encoder(
    ...
    if encoder_type == 'swiglu':
        ...
        return TransformerEncoder(d_model, num_heads, hidden_mult, dropout, causal=causal)
...
# ═══════════════════════════════════════════════════════════════════════════════
# User Dense Gate Fusion (+7k)
# ═══════════════════════════════════════════════════════════════════════════════


class UserDenseGateFusion(nn.Module):
    """Gate-fuses aligned int-dense pairs into extra NS tokens.
    
    For each pair group (e.g. fid 62-66 where int and dense are aligned):
    1. Look up int embedding
    2. log1p(dense) -> project to emb_dim
    3. Gate fusion: fused = int_emb * sigmoid(gate(dense_proj)) + dense_proj
    4. Mean-pool over list dim -> one vector per group
    5. Concat all groups -> project to num_out_tokens * d_model
    
    Also zeros out the corresponding dense positions in user_dense_feats
    before they become the user_dense_token, to avoid information duplication.
    """

    def __init__(self, pair_groups, emb_dim, d_model, num_out_tokens=3):
        super().__init__()
        ...
        self.embs = nn.ModuleList([
            nn.Embedding(g['vocab_size'] + 1, emb_dim, padding_idx=0)
            for g in pair_groups
        ])
        self.dense_projs = nn.ModuleList([
            nn.Linear(1, emb_dim) for _ in pair_groups
        ])
        self.gates = nn.ModuleList([
            nn.Linear(emb_dim, emb_dim) for _ in pair_groups
        ])
        total_pair_dim = emb_dim * len(pair_groups)
        self.agg_proj = nn.Sequential(
            nn.Linear(total_pair_dim, d_model * num_out_tokens),
            nn.LayerNorm(d_model * num_out_tokens),
        )

    def forward(self, user_int_feats, user_dense_feats):
        """Returns (B, num_out_tokens, d_model) gate-fused tokens."""
        pooled_list = []
        for i, g in enumerate(self.pair_groups):
            int_vals = user_int_feats[:, g['int_offset']:g['int_offset'] + g['int_length']].long()
            int_emb = self.embs[i](int_vals)

            den_vals = user_dense_feats[:, g['dense_offset']:g['dense_offset'] + g['dense_length']].float()
            den_vals = torch.log1p(den_vals.clamp(min=0.0))
            den_proj = self.dense_projs[i](den_vals.unsqueeze(-1))

            gate = torch.sigmoid(self.gates[i](den_proj))
            fused = int_emb * gate + den_proj

            int_mask = (int_vals != 0).float().unsqueeze(-1)
            count = int_mask.sum(dim=1).clamp(min=1)
            pooled = (fused * int_mask).sum(dim=1) / count
            pooled_list.append(pooled)

        cat_all = torch.cat(pooled_list, dim=-1)
        out = F.silu(self.agg_proj(cat_all))
        return out.view(out.shape[0], self.num_out_tokens, self.d_model)

    def get_dense_zero_masks(self):
        """Returns list of (dense_offset, dense_length) to zero out in user_dense_feats."""
        return [(g['dense_offset'], g['dense_length']) for g in self.pair_groups]
...
class DINModule(nn.Module):
    """Deep Interest Network: i_pool cross-attends all sequences.
    
    Implementation follows the verified approach:
    - u_pool = mean-pool of all NS tokens
    - i_pool = mean-pool of item NS tokens  
    - i_attn = cross-attention(query=i_pool, kv=all_seq_tokens)
    - output = project(concat(u_pool, i_pool, i_attn)) -> 1 NS token
    """

    def __init__(self, d_model, num_heads=4, dropout=0.0):
        super().__init__()
        self.d_model = d_model
        self.cross_attn = CrossAttention(d_model=d_model, num_heads=num_heads,
    ...
    def forward(self, ns_tokens, seq_tokens_list, seq_padding_masks,
                num_user_total, num_item_ns):
        """
        Args:
            ns_tokens: (B, num_ns, D)
            seq_tokens_list: list of (B, L_i, D)
            seq_padding_masks: list of (B, L_i), True=padding
            num_user_total: user-side NS token count (user_ns + user_dense)
            num_item_ns: item NS token count
        Returns:
            (B, 1, D) DIN token
        """
        u_pool = ns_tokens.mean(dim=1)  # (B, D)
        item_ns = ns_tokens[:, num_user_total:num_user_total + num_item_ns, :]
        i_pool = item_ns.mean(dim=1, keepdim=True)  # (B, 1, D)

        all_seq = torch.cat(seq_tokens_list, dim=1)
        all_mask = torch.cat(seq_padding_masks, dim=1)
        i_attn = self.cross_attn(i_pool, all_seq, key_padding_mask=all_mask)
        ...
        return F.silu(self.out_proj(combined))  # (B, 1, D)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # New modules
        dense_pair_groups: Optional[list] = None,
        use_din: bool = True,
        ...
        self.has_dense_pair = dense_pair_groups is not None and len(dense_pair_groups) > 0
        num_gate_tokens = 0
        if self.has_dense_pair:
            gate_num_out = 3
            self.dense_gate_fusion = UserDenseGateFusion(
                pair_groups=dense_pair_groups, emb_dim=emb_dim,
                d_model=d_model, num_out_tokens=gate_num_out)
            num_gate_tokens = gate_num_out
            logging.info(f"UserDenseGateFusion: {len(dense_pair_groups)} pairs -> {gate_num_out} tokens")

        num_user_total = num_user_ns + (1 if self.has_user_dense else 0)
        self.num_user_total = num_user_total
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                top_k=seq_top_k,
                causal=use_causal_mask if seq_encoder_type == 'transformer' else seq_causal,
        ...
        # DIN module
        if use_din:
            self.din_module = DINModule(d_model=d_model, num_heads=num_heads,
                                        dropout=dropout_rate)
    ...
    def _build_ns_and_seq(self, inputs: ModelInput):
        """Shared logic for forward and predict: build ns_tokens, seq_tokens, masks."""
        # User dense: zero out pair columns before projection (avoid info duplication)
        user_dense = inputs.user_dense_feats
        if self.has_dense_pair:
            user_dense = user_dense.clone()
            for d_off, d_len in self.dense_gate_fusion.get_dense_zero_masks():
                user_dense[:, d_off:d_off + d_len] = 0.0
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = F.silu(self.user_dense_proj(user_dense)).unsqueeze(1)
        ...
        # Gate fusion tokens
        if self.has_dense_pair:
            gate_tokens = self.dense_gate_fusion(inputs.user_int_feats, inputs.user_dense_feats)
            ns_parts.append(gate_tokens)
        ...
        # DIN token
        if self.use_din:
            din_tok = self.din_module(ns_tokens, seq_tokens_list, seq_masks_list,
                                      num_user_total=self.num_user_total,
                                      num_item_ns=self.num_item_ns)
            ns_tokens = torch.cat([ns_tokens, din_tok], dim=1)

        return ns_tokens, seq_tokens_list, seq_masks_list

    def forward(self, inputs: ModelInput) -> torch.Tensor:
        """Runs the forward pass of the PCVRHyFormer model."""
        ns_tokens, seq_tokens_list, seq_masks_list = self._build_ns_and_seq(inputs)
