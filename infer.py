...
def build_model(
    ...
    # Dense pair groups for UserDenseGateFusion
    dense_pair_groups = None
    if model_cfg.get('use_dense_pair', True):
        ...
        int_vocab_map = {}
        for fid, offset, length in dataset.user_int_schema.entries:
            vs = max(dataset.user_int_vocab_sizes[offset:offset + length])
        ...
        if pair_groups:
            dense_pair_groups = pair_groups
            logging.info(f"Infer: detected {len(pair_groups)} dense pair groups")

    # Filter model_cfg to only PCVRHyFormer.__init__ keys
    hyformer_keys = {
        'd_model', 'emb_dim', 'num_queries', 'num_hyformer_blocks', 'num_heads',
        'seq_encoder_type', 'hidden_mult', 'dropout_rate', 'seq_top_k', 'seq_causal',
        'action_num', 'num_time_buckets', 'rank_mixer_mode', 'use_rope', 'rope_base',
        'emb_skip_threshold', 'seq_id_threshold', 'ns_tokenizer_type',
        'user_ns_tokens', 'item_ns_tokens', 'use_din', 'use_causal_mask',
    }
    filtered_cfg = {k: v for k, v in model_cfg.items() if k in hyformer_keys}

    logging.info(f"Building PCVRHyFormer with cfg: {filtered_cfg}")
    model = PCVRHyFormer(
        ...
        dense_pair_groups=dense_pair_groups,
        **filtered_cfg,
    ).to(device)
