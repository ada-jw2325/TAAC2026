...
def build_dense_pair_groups(pcvr_dataset):
    """Auto-detect aligned int-dense pair groups from schema."""
    int_fid_map = {fid: (offset, length)
                   for fid, offset, length in pcvr_dataset.user_int_schema.entries}
    dense_fid_map = {fid: (offset, length)
                     for fid, offset, length in pcvr_dataset.user_dense_schema.entries}
    int_vocab_map = {}
    for fid, offset, length in pcvr_dataset.user_int_schema.entries:
        vs = max(pcvr_dataset.user_int_vocab_sizes[offset:offset + length])
        int_vocab_map[fid] = vs

    pair_groups = []
    for fid in sorted(int_fid_map.keys()):
        if fid not in dense_fid_map:
            continue
        int_off, int_len = int_fid_map[fid]
        den_off, den_len = dense_fid_map[fid]
        if int_len != den_len or int_vocab_map.get(fid, 0) <= 0:
            continue
        pair_groups.append({
            'int_offset': int_off, 'int_length': int_len,
            'dense_offset': den_off, 'dense_length': den_len,
            'vocab_size': int_vocab_map[fid],
        })
        logging.info(f"DensePairGroup: fid={fid}, int_off={int_off}, len={int_len}, vocab={int_vocab_map[fid]}")
    logging.info(f"Total dense pair groups: {len(pair_groups)}")
    return pair_groups if pair_groups else None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_dense_pair', action='store_true', default=True)
    parser.add_argument('--no_dense_pair', dest='use_dense_pair', action='store_false')
    parser.add_argument('--use_din', action='store_true', default=True)
    parser.add_argument('--no_din', dest='use_din', action='store_false')
    parser.add_argument('--use_causal_mask', action='store_true', default=True)
    parser.add_argument('--no_causal_mask', dest='use_causal_mask', action='store_false')
...
def main() -> None:
    ...
    # Build dense pair groups
    dense_pair_groups = None
    if args.use_dense_pair:
        dense_pair_groups = build_dense_pair_groups(pcvr_dataset)

    model_args["dense_pair_groups"] = dense_pair_groups
    model_args["use_din"] = args.use_din
    model_args["use_causal_mask"] = args.use_causal_mask
